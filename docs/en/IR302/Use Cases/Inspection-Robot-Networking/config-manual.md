# Inspection Robot Networking Case Configuration Guide Manual
...
The IR302 series supports Inhand's Device Manager "Device Cloud" management platform, enabling users to achieve remote intelligent device management. The IR302 series is suitable for various industrial and commercial IoT applications, providing efficient and reliable solutions for digital IoT.

### 2.2 Main Functions

- Supports ICS platform, can provide remote maintenance channels for devices
- Supports 4G network, can provide update service channels for downlink devices
- Supports remote configuration, remote diagnosis, remote upgrade
- Industrial-grade design

### 2.3 Typical Application Topology

Inspection robot → IR302 router → 4G → ICS platform → Inspection robot remote service center
...
### 7.3 Changing Local Management Method
...
#### 7.4.2 Settings in ICS Platform

1. Enter ICS platform, after logging in, click [Device Management], add device
![alt text](images/image-14.webp)
2. After configuration, click on the device to enter the terminal settings page
![alt text](images/image-15.webp)
3. Add and configure terminals.

#### 7.4.3 Configure Computer for Remote Maintenance Center

1. Computer logs in to ICS platform, find client download in the upper right corner of the platform
![alt text](images/image-16.webp)
2. Configure the remote maintenance center computer to download and install client software
3. Download Ovpn configuration file from the platform
![alt text](images/image-17.webp)
4. Open the client software and import the Ovpn configuration file
5. Test connection to remote device to confirm successful connection
...
#### Configuration File Information in Attachment

1. /configs/config-IR302.dat: Configuration file, containing default username: adm; password: 123456; LAN IP address: 192.168.2.1.
2. ICS platform configuration file, containing ICS platform account: caoqb@inhand.com.cn.
3. If you need to use this configuration file, it is recommended to modify the corresponding parameters after importing, such as username and password, network parameters, and ICS platform account.
